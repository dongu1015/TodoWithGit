...
const todoController = (() => {
    ...
    const handleToggleTodo = async (e) => {
        ...
        // ✅ 여기에서 모달 띄우기
        const commitMessage = await openCommitModal(todo.title);
        ...
        // ✅ 이때 Django에 데이터 전송
        await sendCommitToDjango(commitMessage, todo);

        todoLocalStorage.populateStorage(todoApp);
    };

    async function sendCommitToDjango(commitMessage, todo) {
        try {
    ...
    function openCommitModal(todoTitle) {
        return new Promise((resolve) => {
            // 1. 백드롭 & 모달 요소 생성
            const overlay = document.createElement('div');
            overlay.className = 'modal-backdrop fade-in';
            ...
            const modal = document.createElement('div');
            modal.id = 'modal'; // SCSS에서 이 id로 스타일 정의되어 있음

            // 2. 내부 구조 구성 - textarea에 resize 속성 직접 적용
            modal.innerHTML = `
        <h2>✅ "${todoTitle}" 완료!</h2>
        <p>커밋 메시지를 입력해주세요:</p>
        <div class="modal-input-section">
      ...
      `;
            ...
            // textarea에 추가 스타일 적용 (JavaScript로도 강제)
            const textarea = modal.querySelector('#commit-input');
            ...
            modal.querySelector('#commit-submit').addEventListener('click', () => {
                const msg = modal.querySelector('#commit-input').value.trim();
                overlay.remove();
                modal.remove();
                resolve(msg);
            });
